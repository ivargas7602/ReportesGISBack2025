...
public class Coneccion { 
	...
	public Connection getConexion() throws Exception {
		if(myConnection==null || myConnection.isClosed())
		{
			try {			
				...
				if(mgs.isEmpty()&&flagWeLogic)
					throw new Exception( "No se ha agregado el parametro de LookupName en web.xml");
				else
					throw new Exception( mgs+"\n"+"No existe conexion con el servidor de Base de Datos" );		 
			}
		}
		return myConnection;
	}
	...
	private String getJdbcUrlOracle(String ip, String bd, String prto) { 
		...
		if(databaseName.length() > 0) 
			...
			url = (new StringBuilder(String.valueOf(url))).append("MYDATABASE").toString(); return url; 
	}
}